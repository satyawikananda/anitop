...
import { OstSongs, DataError } from "../../types/common"

const dataScrape:OstSongs[] = []
const OstSongsScrape = async (url: string) => {
    try{
        ...
        if(response.status == 200) {
            ...
            listAnimes.each((_, el) => {
                ...
                const artists: Array<string> = $(el).find('div.at-mcc-e-details > div.entry-detail').text().trim().split(", ")
